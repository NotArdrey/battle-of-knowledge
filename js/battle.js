...
let sessionCreatedAt = null; // Session safety: track when this battle session started
...
function saveBattleProgress() {
    const battleState = {
        ...
        timestamp: Date.now(),
        // Session safety: store question IDs to handle mid-game content changes
        sessionQuestionIds: questions.map(q => q.id || q.question).filter(Boolean),
...
// Load question with graceful error handling for session safety
function loadQuestion() {
    ...
    if (languageChanged) {
        ...
        if (currentShuffledAnswers.length > 0 && currentQuestion) {
            ...
            currentShuffledAnswers.forEach(oldAnswer => {
                ...
                if (indexInOldAnswers !== -1 && newQuestion && newQuestion.answers) {
    ...
    // Session safety: Handle case where questions array is empty or question unavailable
    if (!questions || questions.length === 0) {
        console.warn('No questions available, attempting to reload from fallback');
        // Try loading from local fallback
        if (questionsData[currentEra] && questionsData[currentEra][currentLanguage]) {
            questions = [...questionsData[currentEra][currentLanguage]];
        } else if (questionsData['early-spanish']) {
            questions = [...questionsData['early-spanish'][currentLanguage] || questionsData['early-spanish']['en']];
        }

        if (!questions || questions.length === 0) {
            console.error('Failed to load questions - showing fallback message');
    ...
    // Session safety: Validate current question exists and has required properties
    if (!currentQuestion || !currentQuestion.question || !currentQuestion.answers) {
        console.warn('Invalid question at index', questionIndex, '- skipping to next');
        questionIndex++;
        if (questionIndex >= questions.length) {
            questionIndex = 0;
            shuffleArray(questions);
        ...
        if (!currentQuestion || !currentQuestion.question) {
            document.getElementById('questionText').textContent = 'Question unavailable - please refresh';
            return;
        }
    }
